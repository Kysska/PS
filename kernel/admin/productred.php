...
?>
    <div class="wrapper">
      <div class="add_box">
        <div class="header_add">Редактирование товара</div>
        <form class="add_box-form"  action = "./productredlogic.php?id=<?=$id?>" method="POST" enctype="multipart/form-data">
        <img src="../imageanimals/<?=$row['image']?>" class="card-img-top" alt="...">  
          </p><p><b>загрузка изображения:</b><br>
            <input type="file" name='avatar' accept="image/*,image/jpeg">
          </p><p><b>название товара:</b><br>
            <input type="text" name="title" value="<?=$row['title']?>"  placeholder="Введите название:" required>

          </p><p><b>цена товара:</b><br>
            <input type="number" name="price" value="<?=$row['price']?>" placeholder="Введите цену:" required>

          </p><p><b>информация о товаре:</b><br>
            <label>описание</label><br>
            <textarea name="desc" cols="40" rows="3" value="<?=$row['description']?>" required><?=$row['description']?></textarea><br>
            <?php
       if($row['tobe'] == 1){?> 
       <label>Скрыть товар:</label>
       <input  type = "radio" class="product-price" name = "check" value="s"></input>
       <?php
       }
       else{
       ?>
       <label>Показать товар:</label>
       <input  type = "radio" class="product-price" name = "check" value="p"></input>
       <?
       }
       ?>
           
        </p> <button type = "submit">Обновить</button></form>
        <?php
              if($_SESSION['message']){
